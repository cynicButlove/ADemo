...
if not exist "%ENGINE_ROOT%\Engine\Binaries\Win64\UnrealEditor-AIGraph.dll" (
	echo Source engine is incomplete: missing UnrealEditor-AIGraph.dll
	echo Expected engine root: %ENGINE_ROOT%
	echo Rebuild the source engine editor modules before using this launcher.
	exit /b 1
)

if not exist "%ENGINE_ROOT%\Engine\Binaries\Win64\UnrealEditor-AITestSuite.dll" (
	echo Source engine is incomplete: missing UnrealEditor-AITestSuite.dll
	echo Expected engine root: %ENGINE_ROOT%
	echo Rebuild the source engine editor modules before using this launcher.
	exit /b 1
)
